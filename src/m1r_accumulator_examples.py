...
#
#              for k in range(r - s):
#                  count = 0                put outside for loop
#                  if math.cos(r) > 0:      math is not imported yet!
#                      count = 1            it needs to add stuff not set count equal to 1
#                  return count             unless you want a running count, put return outside of for loop
#
#     f. The code in the "graphics accumulation" example below includes:
#            for _ in range(n):
#        What does the   _   (underscore) mean?
#           it means the loop is not varible-dependent
...
#
#              x = starting_point.x  this is wrong because all the circles will be in the same spot
#              for k in range(n):
